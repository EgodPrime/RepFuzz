...
import openai
client = openai.OpenAI(base_url="http://192.168.2.29:8021/v1", api_key="yb")
...
class Target(object):
    def __init__(self, language="c", timeout=10, folder="/", **kwargs):
        ...
        self.yb_logger = Logger(self.folder,"f4a_log.txt")
    ...
    # smaller LLMs
    def wrap_prompt(self, prompt: str, api_name: str) -> str:
        raise NotImplementedError
    ...
    def auto_prompt(self, **kwargs) -> str:
        ...
        os.makedirs(output_folder + "/prompts", exist_ok=True)
        ...
        if kwargs["no_input_prompt"]:
            ...
            if not os.path.exists(output_folder + "/prompts/greedy_prompt.txt"):
                ...
                greedy_prompt = self.wrap_prompt(res.choices[0].message.content, api_name)
                with open(
                    output_folder + "/prompts/greedy_prompt.txt", "w", encoding="utf-8"
                ) as f:
            ...
            with open(output_folder + "/prompts/scores.txt", "a") as f:
                f.write(f"greedy score: {str(best_score)}")
            for i in range(3):
                if not os.path.exists(output_folder + "/prompts/prompt_{}.txt".format(i)):
                    res = client.chat.completions.create(
                        model="qwen2.5-instruct-7b",
                        messages=self._create_auto_prompt_message(message),
                        extra_body={"chat_template_kwargs":{"enable_thinking": False}},
                    ...
                    prompt = self.wrap_prompt(res.choices[0].message.content,api_name)
                    with open(
                        output_folder + "/prompts/prompt_{}.txt".format(i),
                        "w",
                        encoding="utf-8",
                    ) as f:
                        f.write(prompt)
                    ...
                    pass
        # dump best prompt
        with open(output_folder + "/prompts/best_prompt.txt", "w", encoding="utf-8") as f:
            f.write(best_prompt)

        return best_prompt

    # initialize through either some templates or auto-prompting to determine prompts
    def initialize(self,api_name, output_folder):
        self.m_logger.logo(
