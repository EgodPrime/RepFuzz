...
class ScikitTarget(Target):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.SYSTEM_MESSAGE = "You are a scikit-learn Fuzzer"
    ...
    def write_back_file(self, code, write_back_name=""):
        if write_back_name != "":
            try:
                ...
                pass
        return "/tmp/temp{}.txt".format(self.CURRENT_TIME)

    def wrap_prompt(self, prompt: str, api_name: str) -> str:
        self.prompt_used['separator'] = f"'''Generate a Python test case using the {api_name} API from the sklearn.feature_extraction.image module.Provide only a runnable Python test case without any explanations or other programming languages.'''"
    ...
    def validate_individual(self, filename) -> Tuple[FResult, str]:
        try:
            ...
            return FResult.TIMED_OUT, "scikit"
